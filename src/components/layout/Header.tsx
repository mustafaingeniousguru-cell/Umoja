...
export default function Header() {
  ...
  return (
    <motion.header
      ...
      className="fixed top-0 left-0 right-0 z-50 px-2 pt-2 pb-3"
    >
      <div
        className={`transition-all duration-500 ${
          scrolled
            ? "bg-[#0a1628]/80 backdrop-blur-2xl border-b border-[#c9a84c]/10 py-3"
            : "bg-transparent py-4"
        }`}
        style={
          scrolled
      ...
      >
        <div className="max-w-[1600px] mx-auto px-3 sm:px-8 lg:px-12 flex items-center justify-between gap-2 sm:gap-4">
          ...
          <nav className="hidden lg:flex items-center gap-6 xl:gap-8 flex-1 justify-start pl-4 xl:pl-8">
            {leftNavLinks.map((link) => (
              <a
                key={link.label}
                href={link.href}
                className="text-sm lg:text-base xl:text-lg text-white hover:text-[#c9a84c] transition-colors duration-300 relative group whitespace-nowrap"
              >
                {link.label}
          ...
          {/* Right side — Members nav + Wishlist/Cart/SignIn */}
          <div className="flex items-center gap-6 xl:gap-8 flex-1 justify-end pr-4 xl:pr-8">
            ...
            <div className="hidden md:flex items-center gap-4 lg:gap-5">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.97 }}
                className="flex items-center gap-2 px-4 lg:px-5 py-2 lg:py-2.5 rounded-full border border-[#c9a84c]/30 text-white hover:bg-[#c9a84c] hover:text-[#0a1628] font-bold text-xs lg:text-sm xl:text-base uppercase tracking-[0.15em] transition-all duration-300 cursor-pointer whitespace-nowrap"
              >
                Sign In
              </motion.button>
            </div>
