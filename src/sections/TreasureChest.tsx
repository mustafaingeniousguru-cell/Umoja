...
export default function TreasureChest() {
  return (
    <section id="bundles" className="relative w-full py-12 lg:py-32 overflow-hidden border-t-[4px] border-[#c9a84c]/20"
      style={{
    ...
    >
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10">
        ...
        <div className="text-center mb-6 sm:mb-16">
          ...
          >
            <div className="text-[10px] font-black tracking-[0.15em] text-[#c9a84c] uppercase mb-2 sm:mb-4 drop-shadow-md">
              Curated Collections
            </div>
            <h2 className="text-xl sm:text-5xl lg:text-6xl font-serif text-white uppercase tracking-wider drop-shadow-lg font-bold mb-3 sm:mb-6">
              Treasure Chests
            </h2>
            <p className="text-white/90 max-w-[600px] mx-auto text-xs sm:text-lg font-light leading-relaxed">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-8 lg:gap-10 mt-6 sm:mt-10">
          {bundles.map((bundle, idx) => {
            ...
            return (
              <motion.div
                ...
                className={`relative flex flex-col p-4 sm:p-8 lg:p-10 rounded-2xl bg-[#0a1628]/80 backdrop-blur-xl border-[2px] transition-all duration-300 group
              ...
              >
                ...
                <div className={`text-[9px] sm:text-xs font-black tracking-widest mb-3 sm:mb-6 ${isCollector ? 'text-[#c9a84c]' : 'text-white/80'}`}>
                  {bundle.badge}
                </div>

                {/* Title */}
                <h3 className="text-lg sm:text-3xl font-serif font-bold text-white mb-2 sm:mb-4">
                  {bundle.title}
                </h3>
                ...
                {/* Description */}
                <p className="text-white/95 text-[11px] sm:text-base leading-relaxed mb-4 sm:mb-8 min-h-[60px]">
                  {bundle.description}
                </p>
                ...
                <div className="mb-4 sm:mb-8">
                  <div className="flex items-baseline gap-2 sm:gap-3">
                    <span className={`text-xl sm:text-4xl font-bold ${isCollector ? 'text-[#c9a84c]' : 'text-white'}`}>
                      {bundle.price}
                    </span>
                ...
                {/* Checkmarks / Features */}
                <div className="flex flex-col gap-2 sm:gap-4 mb-6 sm:mb-10 flex-grow">
                  {bundle.features.map((feature, i) => (
                    <div key={i} className="flex items-start gap-2 sm:gap-3">
                      <div className="w-1.5 h-1.5 rounded-full bg-[#c9a84c] shrink-0 mt-1.5 sm:mt-2" />
                      <span className="text-xs sm:text-base text-white">{feature}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="mt-auto">
                  <button className={`w-full py-2.5 sm:py-4 rounded font-bold uppercase tracking-wider text-[11px] sm:text-sm transition-all duration-300
                    ${isCollector 
